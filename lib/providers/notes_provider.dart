...
class NotesProvider extends ChangeNotifier {
  ...
  void duplicateNote(String id) {
    _duplicateNoteRecursive(id, null);
    _save();
    notifyListeners();
  }

  String? _duplicateNoteRecursive(String id, String? newParentId) {
    final original = _notes[id];
    if (original == null) return null;

    final newId = _uuid.v4();
    final newChildIds = <String>[];
    final newChildPreviews = <String, String>{};

    for (final childId in original.childIds) {
      final newChildId = _duplicateNoteRecursive(childId, newId);
      if (newChildId != null) {
        newChildIds.add(newChildId);
        if (original.childPreviews.containsKey(childId)) {
          newChildPreviews[newChildId] = original.childPreviews[childId]!;
        }
      }
    }
    ...
    final newNote = Note(
      ...
      childIds: newChildIds,
      childPreviews: newChildPreviews,
      images: List.from(original.images),
      pinned: original.pinned, // Alternatively, false to unpin duplicates
      isList: original.isList,
      isDeleted: original.isDeleted,
      isArchived: original.isArchived,
    );
    
    _notes[newId] = newNote;

    if (newParentId == null) {
      // Add right after the original or just at the top. Top is easier:
      _rootIds.insert(0, newId);
    }
